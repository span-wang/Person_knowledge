...
export interface PracticeService {
  ...
  answer(sessionId: string, questionId: string, request: PracticeAnswerRequest): Promise<PracticeSessionResponse>;
...
async function sessionRows(database: PracticeSqlExecutor, sessionId: string) {
  ...
  const [sessionResult, attemptResult] = await Promise.all([
    database.execute('SELECT id, question_bank_id, question_chapter_id, mode, source, status, started_at, completed_at, updated_at FROM practice_sessions WHERE id = ? LIMIT 1', [id]),
    database.execute('SELECT question_id, question_version, sort_order, snapshot_json, answer_json, result, answered_at FROM practice_attempts WHERE practice_session_id = ? ORDER BY sort_order, id', [id]),
  ]);
  ...
  return {
    ...
    async start(request) {
      ...
      return transaction(database, async (connection) => {
        ...
        let questionSql = 'SELECT q.id, q.question_bank_id, q.question_chapter_id, q.stem_json, q.question_type, q.options_json, q.answer_json, q.analysis_json, q.version FROM questions AS q';
        ...
        const questions = rowsFrom(questionRows);
        if (!questions.length) throw new PracticeApiError(409, '当前范围没有可练习的题目。');
        const sessionId = randomUUID();
        await connection.execute('INSERT INTO practice_sessions (id, question_bank_id, question_chapter_id, mode, source, scope_json, status) VALUES (?, ?, ?, ?, ?, ?, \'in_progress\')', [sessionId, bank.id, chapterId, mode, source, JSON.stringify({ questionIds: questions.map((row) => textValue(row.id)), sourceSessionId: request.sourceSessionId ?? null })]);
    ...
    async answer(sessionId, questionId, request) {
      return transaction(database, async (connection) => {
        ...
        const attempt = loaded.attempts.find((row) => textValue(row.question_id) === requiredId(questionId, '题目标识'));
        if (!attempt) throw new PracticeApiError(404, '题目不在当前会话中。');
        ...
        return viewFromRows(next.session, next.attempts, mode, 'in_progress');
      });
    },
